...
| 55 | **Homebrew cask** | Done for early builds: `superkeys` 0.1.0 is in the tap (unsigned, hosted at superkeys.space/download/). Tap created 2026-09-24: public repo [`Rxmeez/homebrew-tap`](https://github.com/Rxmeez/homebrew-tap), shared by all projects (`Casks/superkeys.rb`, later other casks and `Formula/`), installed with `brew trust --tap rxmeez/tap` then `brew install --cask rxmeez/tap/superkeys`. Adding `Casks/superkeys.rb` waits on a signed download (#16, #64). Move to plain `brew install --cask superkeys` if homebrew/cask accepts it; the tap keeps serving everything else. Note the established app **Superkey** (superkey.app) is already cask `superkey`, one letter away. | S |
| 56 | **Updates** | In progress: Sparkle in-app updates from superkeys.space/appcast.xml, EdDSA-signed, daily checks, "Check for Updates…" in the menu and a toggle in General. Updates the app downloads itself aren't quarantined, so no Gatekeeper prompt, and the stable "Superkeys" certificate keeps Accessibility. Keys live in 1Password; `scripts/release.sh` publishes. Remaining: run the key setup, ship 0.1.1, verify an update end to end, then a GitHub Actions release job reading the same secrets. | M |
...
| 69 | **Rename the signing identity** | Release builds are signed by the self-signed certificate "Hypercaps Dev", which Gatekeeper shows as the origin. Issue a "Superkeys" certificate before there are many users, since changing it makes every existing install re-grant Accessibility once. | S |
| 70 | **"Opening Superkeys" help page** | Testing 0.1.1 showed how easy the first-launch warning is to get wrong: its highlighted button is "Move to Bin", and "Open Anyway" is at the bottom of Privacy & Security, below a long list. A `superkeys.space/install` page with three screenshots (click Done, scroll to Security, Open Anyway, then Accessibility), linked from the cask caveats, the README, and the site's install box. | S |
