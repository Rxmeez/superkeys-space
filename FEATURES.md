...
| 89 | **A config you already have** | On a new Mac with dotfiles, `~/.config/superkeys/config.toml` is already there before first launch. The welcome tour should notice ("Found your config: 9 app keys, 3 keystrokes") and skip the app-suggestion step instead of offering keys the file already assigns differently. `ConfigSync.start()` already loads the file before the tour's apps step runs; the step only needs to read what's there. | S |
| 90 | **✦ ↓ centres the window** | ✦ ↓ is the one window arrow that does nothing. Centre the focused window at a comfortable size (about two thirds of the screen), and again to restore, mirroring ✦ ↩ for fill. Reuses `WindowManager`'s saved frames. | S |
| 91 | **Show the waiting group on the icon** | Between ✦ O and the second key, only the group panel (after 0.35 s) says anything is pending. The menu bar icon could show the first key's letter while it waits, so a quick typist who pauses knows why nothing happened yet. | S |
...
| 92 | **Keystroke sequences** | One ✦ key sending two or three combinations in order, e.g. ✦ L → ⌘ C, ⌘ Tab, ⌘ V to copy something into the previous app. In config.toml a list: `L = ["cmd+c", "cmd+tab", "cmd+v"]`. Needs a short pause between steps so the target app keeps up, and stays small on purpose (no conditions, no loops) so it doesn't turn into a scripting language. | M |
| 93 | **Machine-specific keys** | One config in dotfiles, used on a work and a home Mac with different apps. `[apps]` could sit under `[machine."Work-MacBook"]` (the computer name), overriding the shared keys there. Unknown apps are already skipped with a note, so this mostly helps when the same key should mean different apps. | M |
| 94 | **Keys you never use** | Settings → Shortcuts could fade out app keys and keystrokes that haven't been pressed in 30 days, to make space for new ones. Counts kept locally in preferences, never sent, and resettable; the privacy page should say so. | S |
| 95 | **VoiceOver for groups** | When the group panel opens, announce its choices ("O group: P 1Password, M Mail"), and read keystrokes as "Hyper C sends Control C". Complements #38, which covers window and desktop actions. | S |
...
| 17 | **More tests** | Unit tests for the remap merge, key labels, key validation, snap geometry, the config.toml reader (round trips, problems, syntax errors) and the keystroke tap logic (the DEBUG `selfTestKeystrokes` harness is a ready-made starting point). The remap merge was verified by hand this time; it should not have to be again. | M |
| 67 | **Build check on every push** | A GitHub Actions job on a macOS runner that builds the app (ad hoc signing, no secrets) and runs the tests from #17 (`SuperkeysTests`, 14 so far, `xcodebuild -scheme Superkeys test`) for every push and pull request, so a broken build is caught before a release tag, not during it. Also a place to run `swift-format --lint`. Private repos get 2,000 free Actions minutes a month; macOS minutes count 10×, so keep it to one build per push. | S |
...
- **Tests (first part of #17).** `SuperkeysTests`, hosted in the Dev app (which skips startup under XCTest via `TestRun.active`): config.toml reading/writing/problems and the `[keys]` upgrade (#83), keystroke parsing, key handling for keystrokes and groups through the DEBUG self-tests, and Settings list grouping. `scripts/release.sh` refuses to ship if they fail. Also built: `[keys]` added to older config files (#83) and `~/.config/superkeys` in the cask's zap (#84).
- **Key groups.** ✦ + key, then a second key (✦ O then P) for apps and keystrokes. The first key keeps its own binding, run on ✦ release when no second key came, so there is no timeout and non-group keys stay instant. A key outside the group cancels it and acts normally; Esc cancels. Group panel after 0.35 s; `"O P"` in config.toml; "+ then…" in the recorder. The ✦ panel lists keystrokes too (was #78). Two keys at most. DEBUG `selfTestGroups` covers the five paths.
