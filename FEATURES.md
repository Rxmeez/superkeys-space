...
| 55 | **Homebrew cask** | One shared, public tap repo for all projects: `Rxmeez/homebrew-tap` (`Casks/superkeys.rb`, later other casks and `Formula/`), installed as `brew install --cask rxmeez/tap/superkeys`. Move to plain `brew install --cask superkeys` if homebrew/cask accepts it; the tap keeps serving everything else. Note the established app **Superkey** (superkey.app) is already cask `superkey`, one letter away. | S |
