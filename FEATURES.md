...
| 77 | **Terminal keystrokes in one click** | The empty Keystrokes list offers "Add ✦ C / D / R as ⌃ C / D / R", the set that made ✦-as-Control worth building. Off until clicked, so ✦ keys stay silent by default. | S |
...
| 79 | **A terminal step in the tour** | When the tour's most-used apps include a terminal, offer the ✦ C / D / R keystrokes as an optional step after app keys, with the same keyboard drawing lighting ⌃ as ✦ is held. Skipped for everyone else. | S |
